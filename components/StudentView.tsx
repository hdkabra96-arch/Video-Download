...
const StudentView: React.FC<StudentViewProps> = ({ papers, activeStudent, onLogin, registeredStudents, onSubmission }) => {
  ...
  const handleInsertTable = (rows: number, cols: number) => {
    ...
    for (let i = 0; i < cols; i++) {
      tableHtml += `    <th style="border:1px solid #333;padding:8px;background:#f0f0f0;font-weight:bold;text-align:center;">Header ${i + 1}</th>\n`;
    }
    tableHtml += '  </tr>\n';
    ...
    for (let r = 0; r < rows - 1; r++) {
      ...
      for (let c = 0; c < cols; c++) {
        tableHtml += `    <td style="border:1px solid #ddd;padding:8px;text-align:center;">[ ]</td>\n`;
      }
      tableHtml += '  </tr>\n';
    }
    ...
    const currentAnswerText = activeSession?.answers[activeSession.paper.questions[currentQuestionIndex].id]?.answerText || '';
    updateAnswer(currentAnswerText + tableHtml);
            ...
            <div className="max-w-3xl mx-auto bg-white rounded-3xl p-8 border border-slate-200 shadow-sm">
               ...
               {currentQ.type === 'mcq' && currentQ.options ? (
                 ...
                 <div className="space-y-4">
                   ...
                   />
                   {currentAnswer?.answerText.includes('<table') && (
                     <div className="bg-slate-100 rounded-2xl p-6 border border-slate-200">
                       <p className="text-xs font-bold text-slate-500 mb-3 uppercase tracking-widest">📊 Table Preview</p>
                       <div 
               ...
               <div className="mt-8 pt-8 border-t border-slate-100">
                  <div className="flex flex-wrap items-center gap-3 mb-4">
                     <label className="flex items-center gap-3 bg-slate-50 border border-slate-200 px-4 py-3 rounded-2xl cursor-pointer hover:bg-slate-100 transition-colors">
                        ...
                        <span className="text-xs font-black uppercase text-slate-600">Capture</span>
                  ...
                  <div className="flex justify-end items-center gap-3">
                     ...
                     {currentAnswer?.answerText && (
                       <span className="text-xs font-bold text-blue-600">✓ Text ({currentAnswer.answerText.length} chars)</span>
                     )}
                  </div>
               </div>
            </div>
