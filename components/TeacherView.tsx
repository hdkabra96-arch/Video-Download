...
interface TeacherViewProps {
  ...
  onPaperUpdated: (paper: QuestionPaper) => Promise<'success' | 'saved_without_file' | 'failed'>;
  onPaperDeleted: (id: string) => void;
  existingPapers: QuestionPaper[];
  submissions: Submission[];
}

const TeacherView: React.FC<TeacherViewProps> = ({ onPaperCreated, onPaperUpdated, onPaperDeleted, existingPapers, submissions }) => {
  ...
  const savePaper = async () => {
    ...
    const editingPaper = editingPaperId ? existingPapers.find(p => p.id === editingPaperId) : null;
    const paperToSave: QuestionPaper = editingPaper
      ? {
          ...editingPaper,
          title,
          subject,
          ...
          duration,
          questions,
          // Keep existing attachment unless a new file was uploaded
          pdfData: uploadedFile?.data ?? editingPaper.pdfData,
        }
      : { id: `paper-${Date.now()}`, title, subject, grade, duration, questions, createdAt: nowIso, pdfData: uploadedFile?.data };

    const result = editingPaper ? await onPaperUpdated(paperToSave) : await onPaperCreated(paperToSave);
    setIsSaving(false);
    if (result !== 'failed') {
      if (result === 'success') {
        alert(editingPaper ? "✅ Exam updated successfully!" : "✅ Exam Published successfully!");
      } else if (result === 'saved_without_file') {
        alert(
          (editingPaper ? "✅ Exam updated!" : "✅ Exam Published!") +
            "\n\n⚠️ Note: File attachment was too large for cloud storage but has been saved locally. Students can still access the exam."
        );
      }
      ...
      alert(editingPaper ? "❌ Failed to update exam. Please try again." : "❌ Failed to publish exam. Please try again.");
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const needsCompression = isPdf && file.size > 5 * 1024 * 1024;

      let finalBlob: Blob = file;
      let finalName = file.name;
      let finalType = file.type || 'application/pdf';

      if (needsCompression) {
        setIsCompressing(true);
        const inputBytes = await file.arrayBuffer();
        const result = await compressPdfToTarget(inputBytes, {
          targetBytes: 3 * 1024 * 1024,
          maxBytes: 3 * 1024 * 1024,
          maxPages: 40, // safety cap; most exams are well under this
        });
      ...
      const base64Full: string = await new Promise((resolve, reject) => {
      ...
      setUploadedFile({ name: finalName, data: base64Full, type: finalType, size: finalBlob.size });

        if (process.env.API_KEY) {
          setIsAnalyzing(true);
          try {
            if (finalBlob.size > 20 * 1024 * 1024) {
            ...
            const response = await ai.models.generateContent({
              ...
              contents: { parts: [{ inlineData: { mimeType: finalType || 'application/pdf', data: base64Full.split(',')[1] } }, { text: `Extract: title, subject, grade, and questions list.` }] },
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => {
                        ...
                        setQuestions((paper.questions || []).map(q => ({ ...q })));
                        setUploadedFile(null);
                        setPreviewUrl(null);
                        setShowCreator(true);
                      }}
                      className="bg-slate-100 text-slate-800 px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-slate-200 transition-colors"
                    >
                      Edit
                    </button>
                  </div>
        ...
        <div className="space-y-6">
          {submissions && submissions.length > 0 ? (
            submissions.map(submission => {
              // Find the original paper to get question order
              const originalPaper = existingPapers.find(p => p.id === submission.paperId);
              // Build ordered answers: use paper question order if available, fallback to Object.entries
              const orderedAnswers: { questionId: string; questionText: string; answer: typeof submission.answers[string] }[] = [];
              if (originalPaper && originalPaper.questions.length > 0) {
                originalPaper.questions.forEach((q) => {
                  const answer = submission.answers[q.id];
                  if (answer) {
                    orderedAnswers.push({ questionId: q.id, questionText: q.text, answer });
                  }
                });
                // Also include any answers not matched to a question (edge case)
                Object.entries(submission.answers).forEach(([qId, answer]) => {
                  if (!orderedAnswers.find(oa => oa.questionId === qId)) {
                    orderedAnswers.push({ questionId: qId, questionText: '', answer });
                  }
                });
              } else {
                Object.entries(submission.answers).forEach(([qId, answer]) => {
              ...
              return (
                <div key={submission.id} className="bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm hover:shadow-xl transition-shadow">
                            ...
                            <html>
                              <head>
                                ...
                                <style>
                                  ...
                                  .question-text { color: #374151; font-weight: bold; margin-bottom: 8px; font-style: italic; }
                              ...
                              <body>
                                ...
                                ${orderedAnswers.map(({ questionText, answer }, idx) => `
                                  <div class="question">
                                    <h2>Question ${idx + 1}</h2>
                                    ${questionText ? `<p class="question-text">Q: ${questionText}</p>` : ''}
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-200">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Answers ({orderedAnswers.length} Questions)</p>
                    <div className="bg-slate-50 rounded-2xl p-6 max-h-96 overflow-y-auto space-y-4">
                      {orderedAnswers.map(({ questionId, questionText, answer }, idx) => (
                        <div key={questionId} className="border-b border-slate-200 pb-4 last:border-0">
                          <p className="text-xs font-bold text-indigo-600 uppercase tracking-widest mb-1">Question {idx + 1}</p>
                          {questionText && (
                            <p className="text-xs text-slate-500 italic mb-2">{questionText}</p>
                          )}
                          {answer.answerText && (
          ...
          <div className={`h-full flex gap-8 ${previewUrl ? 'max-w-full' : 'max-w-4xl mx-auto'}`}>
            ...
            <div className="flex-1 space-y-8">
              <div className="flex justify-between items-center">
                <h2 className="text-3xl font-black">{editingPaperId ? 'Edit Exam' : 'Configure Exam'}</h2>
                <button
                  onClick={() => { setShowCreator(false); setEditingPaperId(null); }}
                  className="p-2"
                >
              ...
              {editingPaperId && (
                <div className="bg-amber-50 border border-amber-200 text-amber-900 rounded-2xl p-5 text-xs font-bold">
                  You are editing an existing exam. If you don’t select a new file, the current attachment (if any) will be kept.
                </div>
              )}
              <div className="p-8 bg-slate-900 rounded-[2rem] text-white">
                ...
                <button onClick={() => fileInputRef.current?.click()} disabled={isAnalyzing || isCompressing} className="bg-white text-slate-900 px-8 py-4 rounded-xl font-black uppercase text-xs flex items-center gap-3 disabled:opacity-60">
                  {(isAnalyzing || isCompressing) ? <div className="w-4 h-4 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin"></div> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6" /></svg>}
                  {isCompressing ? 'Compressing PDF...' : (isAnalyzing ? 'Scanning Document...' : 'Select Question Paper')}
                </button>
              ...
              <button onClick={savePaper} disabled={isSaving} className="w-full bg-indigo-600 text-white py-6 rounded-2xl font-black uppercase tracking-widest shadow-2xl disabled:opacity-50">{isSaving ? 'Processing...' : (editingPaperId ? 'Save Changes' : 'Publish Exam')}</button>
            </div>
          </div>
        </div>
      )}
